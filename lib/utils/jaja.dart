...
class DbHelper {
  ...
  static final _tableName = 'userImage';
...
// Prints a list that include Fido.
  ...
  Future<int> update(Map<String, dynamic> row) async {
    Database db = await insatance.database;
    int id = row[columnImageId];
    return await db
        .update(_tableName, row, where: '$columnImageId = ?', whereArgs: [id]);
  }

  Future<int> delete(int id) async {
    Database db = await insatance.database;
    return await db
        .delete(_tableName, where: '$columnImageId=?', whereArgs: [id]);
  }
